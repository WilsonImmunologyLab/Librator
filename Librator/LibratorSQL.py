...
def creatnewDB(DBpathname):
    ...
    cursor.execute("create table LibDB(SeqName text PRIMARY KEY NOT NULL, Sequence text, SeqLen, SubType text, Form text, Placeholder text, ID)")
    # ID PRIMARY KEY,
...
def UpdateField(ID, Value, Field, DBpathname):
    ...
    # cursor.execute('drop table if exists LibDB')
    nID = str(ID)
    SQLCommand = 'UPDATE LibDB SET ' + Field + ' = "' + Value + '" WHERE ID = ' + nID
    # if Field == 'SeqAlignment':
    #     SQLCommand = 'UPDATE LibDB SET Isotype = "' + Value + '" WHERE ID = ' + ID
    try:
        cursor.execute(SQLCommand)
...
def enterData(DBpathname, LibratorSeqs):
    ...
    if len(FinalBLASTed) > 0:
        cursor.executemany('''INSERT INTO LibDB(SeqName, Sequence, SeqLen, SubType, Form, Placeholder, ID) VALUES(?,?,?,?,?,?,?)''', FinalBLASTed)
